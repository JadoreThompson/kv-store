...
public record RedirectMessage(
    ...
    public byte[] serialize() {
        ...
        String host = node.serverAddress().getHostString();
        byte[] hostBytes = host.getBytes(StandardCharsets.UTF_8);

        int port = node.serverAddress().getPort();
    ...
    public static RedirectMessage deserialize(ByteBuffer buffer) {
        try {
            ...
            NodeConfig node = new NodeConfig(name, new InetSocketAddress(host, port), null);
