package com.zenz.kvstore.server.raft.server.handler;
...
public class RaftBrokerServerHandler implements SocketHandler {
    ...
    public RaftBrokerServerHandler(SocketServer server, RaftManager manager) {
        ...
        this.server = server;
        DEBUG_PREFIX = String.format("[nodeId=%s RaftBrokerServer]", manager.getNodeConfig().name());
    }
...
//            if (msg.term() > manager.getLastSeenTerm() && msg.term() > manager.getLastVotedTerm()) {
//                manager.setLastSeenTerm(msg.term());
//
//                if (msg.prevLogId() >= manager.getKVStore().getLogHandler().getLogId()) {
//                    manager.setLastVotedTerm(msg.term());
