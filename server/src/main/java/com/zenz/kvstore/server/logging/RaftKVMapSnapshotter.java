...
public class RaftKVMapSnapshotter implements Snapshotter<RaftLogEntry> {
    ...
    public void restore(final KVStore kvStore) throws IOException {
        ...
        final Logger prevLogger = kvStore.getLogHandler().getLogger();
        final Snapshotter<?> prevSnapshotter = kvStore.getLogHandler().getSnapshotter();
        ...
        kvStore.getLogHandler().setLogger(logger);
        kvStore.getLogHandler().setSnapshotter(snapshotter);
        ...
        for (File file : files) {
            try (final FileInputStream fis = new FileInputStream(file)) {
                ...
                final RaftLogHandler logHandler = new RaftLogHandler(logger, snapshotter);
                logHandler.setEnabled(false);
                logHandler.setLogId(header.firstLogId - 1);
                logHandler.setTerm(header.firstLogTerm);
                kvStore.setLogHandler(logHandler);
        ...
        kvStore.getLogHandler().setLogger(prevLogger);
        kvStore.getLogHandler().setSnapshotter(prevSnapshotter);
    }
